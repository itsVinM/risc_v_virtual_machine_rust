...
fn disassemble(raw: u32) -> String {
    let inst = decode(raw);
    fn rn(r: usize) -> &'static str { Cpu::reg_name(r) }
    match inst {
        // Lui/Auipc: display page number (upper 20 bits)
        Inst::Lui   { rd, imm }      => format!("lui     {}, 0x{:x}", rn(rd as usize), (imm as u64) >> 12),
        Inst::Auipc { rd, imm }      => format!("auipc   {}, 0x{:x}", rn(rd as usize), (imm as u64) >> 12),
        // Addi: display li/mv pseudo-instructions
        Inst::Addi  { rd, rs1, imm } if rs1 == 0 => format!("li      {}, {}", rn(rd as usize), imm),
        Inst::Addi  { rd, rs1, imm } if imm == 0 => format!("mv      {}, {}", rn(rd as usize), rn(rs1 as usize)),
        Inst::Addi  { rd, rs1, imm } => format!("addi    {}, {}, {}", rn(rd as usize), rn(rs1 as usize), imm),
        // Everything else: generated by build.rs from // instr("...") comments
        _ => inst.disassemble_inner(rn),
    }
}
