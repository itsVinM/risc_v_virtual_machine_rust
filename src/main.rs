...
type Bus = Mmu;
use crate::cpu::csr::{MSTATUS_SIE, MSTATUS_SPIE, MSTATUS_SPP};
...
fn load_elf(bytes: &[u8]) -> Option<(Vec<u8>, u64)> {
    if bytes.len() < 64 || bytes.get(0..4)? != [0x7f, b'E', b'L', b'F'] {
        return None;
    }
    if *bytes.get(4)? != 2 {
        return None;
    ...
    for idx in 0..phnum {
        ...
        if p_vaddr < DRAM_BASE as usize || p_vaddr + p_memsz > DRAM_END as usize {
            return None;
        }
...
fn print_state(cpu: &Cpu, bus: &Bus, bps: &HashSet<u64>) {
    ...
    println!("\n{CYN}── status ───────────────────────────────────────────────────────────{RST}");
    println!(
        "  halted={:<5}  mode={:?}  breakpoints={}  cycle={}  instret={}",
        cpu.halted,
        cpu.priv_level,
        bps.len(),
        cycle,
...
fn vm_tick(cpu: &mut Cpu, bus: &mut Bus) -> StepResult {
    if bus.clint.timer_pending() {
        cpu.csr.set_mip_timer();
    } else {
        cpu.csr.clear_mip_timer();
    }
    ...
    if bus.has_pending_plic_s() {
        cpu.csr.set_mip_seip();
    } else {
        cpu.csr.clear_mip_seip();
    }
    // CLINT: software interrupt → mip.SSIP
    if bus.clint.softint_pending() {
        cpu.csr.set_mip_ssip();
    } else {
        cpu.csr.clear_mip_ssip();
    }
...
fn run_debugger(cpu: &mut Cpu, bus: &mut Bus, _initial_entry: u64) {
    ...
    loop {
        ...
        if cpu.halted {
            println!("{YLW}VM halted. Use 'reset' to restart.{RST}");
            break;
        }
        let cmd = prompt(&bps);
        ...
        match verb {
            ...
            "reg" => {
                ...
                if rname.is_empty() {
                    ...
                    let idx = (0..32).find(|&i| {
                        Cpu::reg_name(i) == rname
                            || format!("x{i}") == rname
                            || (rname.starts_with('x')
                                && rname[1..].parse::<usize>().ok() == Some(i))
                            || i.to_string() == rname
                    });
                    match idx {
                        Some(i) => {
            ...
            "h" | "help" => help_msg(),
            "q" | "quit" => break,
            other if !other.is_empty() => println!("{DIM}  unknown '{other}'. try h for help{RST}"),
            _ => {}
        }
    }
}
    ...
    loop {
        match vm_tick(cpu, bus) {
            StepResult::Halted => {
                ...
                println!(
                    "halted  pc={:#010x}  a0={}  cycles={}",
                    cpu.pc,
                    cpu.regs[10] as i64,
                    cpu.csr.read(CSR_CYCLE, Privilege::M)
                );
                break;
            }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if smode {
        ...
        if debug {
            run_debugger(&mut cpu, &mut bus, entry);
        } else {
            run_headless(&mut cpu, &mut bus);
        }
        return Ok(());
    }
